...
- Precios: cargados en córdobas (C$) desde el catálogo de marca (`Catalogo The Ritual.pdf`) y **se muestran** en la card, el modal, el carrito (total por línea + subtotal del pedido) y el mensaje de WhatsApp. Formato vía `formatPrice()` en `lib/site.ts` (`C$ 145`, `C$ 147.50`). **Cold Brew clásico** no está en el catálogo → sigue `price: null` y la UI muestra "A confirmar" (no suma al subtotal). No inventar montos: lo que falte queda `null`. El **total final con envío** lo cotiza Fermento por WhatsApp (la tarifa de delivery no está definida), así que el carrito muestra **subtotal de productos**, no un total cerrado.
